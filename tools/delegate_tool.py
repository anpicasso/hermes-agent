...
def delegate_task(
    ...
    # ---- Background mode: fire-and-forget (fire → register → return immediately) ----
    if background:
        if len(task_list) != 1:
            return json.dumps(
        ...
        # Resolve toolsets: use gateway-set toolsets > parent's enabled toolsets
        toolsets_to_pass = getattr(parent_agent, "_gateway_toolsets", None)
        if not toolsets_to_pass:
            toolsets_to_pass = getattr(parent_agent, "enabled_toolsets", None) or []
        ...
        # Register in the registry (this adds to pending list for gateway drain)
        session = bg_delegation_registry.register(
            prompt=prompt,
            source=source,
            task_id=f"bg_delegate_{uuid.uuid4().hex[:10]}",
            toolsets=toolsets_to_pass,
        )
        ...
        def _bg_thread():
            ...
            _parent_tool_names = list(_mt._last_resolved_tool_names)
            child._delegate_saved_tool_names = _parent_tool_names

            try:
                result = _run_single_child(0, t["goal"], child, parent_agent)
                bg_delegation_registry.mark_done(session.id, result=result)
            except Exception as exc:
                logger.exception(f"[bg-delegate] background delegation failed")
                bg_delegation_registry.mark_done(session.id, error=str(exc))
            finally:
                _mt._last_resolved_tool_names = _parent_tool_names

        thread = threading.Thread(
            target=_bg_thread, daemon=True, name=f"bg-delegate-{session.id}"
        )
        thread.start()
